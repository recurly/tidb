...
func buildSelectField(tctx *tcontext.Context, db *BaseConn, dbName, tableName string, completeInsert bool) (string, int, error) { // revive:disable-line:flag-parameter
	...
	for _, oneRow := range results {
		...
		if fieldType == "date" ||
			...
			hasFloatColumn = true
			checksumSql = fmt.Sprintf("ifnull(if(round(cast(%s as DECIMAL(38,9)), 3) like '%%.%%', trim(trailing '.' from trim(trailing '0' from round(cast(%s as DECIMAL(38,9)), 3))), round(cast(%s as DECIMAL(38,9)), 3)), '')", escapedField, escapedField, escapedField)
